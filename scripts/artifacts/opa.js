...
export class OpaCliError extends Error {
  constructor(message, { code = 'OPA_CLI_FAILED', exitCode = null, stdout = '', stderr = '' } = {}) {
    super(message);
...
export function validateWorkspacePath(path) {
  if (typeof path !== 'string' || !path.trim()) throw new OpaWorkspaceError('Workspace file path must be a non-empty string');
  if (path.includes('\0')) throw new OpaWorkspaceError(`Workspace path contains NUL: ${JSON.stringify(path)}`);
  if (path.includes('\\')) throw new OpaWorkspaceError(`Workspace path must use forward slashes: ${path}`);
  if (isAbsolute(path) || /^[A-Za-z]:/.test(path) || path.startsWith('/')) throw new OpaWorkspaceError(`Absolute workspace paths are not allowed: ${path}`);
  const parts = path.split('/');
  if (parts.some(part => !part || part === '.' || part === '..')) throw new OpaWorkspaceError(`Unsafe workspace path: ${path}`);
  if (parts.some(part => part.length > 255)) throw new OpaWorkspaceError(`Workspace path segment is too long: ${path}`);
  if (path.length > 1024) throw new OpaWorkspaceError(`Workspace path is too long: ${path}`);
  const extension = extname(path).toLowerCase();
  if (!SUPPORTED_EXTENSIONS.has(extension)) throw new OpaWorkspaceError(`Unsupported OPA workspace file type: ${path}`);
  return path;
}
...
function assertWorkspaceReference(value, name, files) {
  ...
  if (!Object.hasOwn(files, path)) throw new OpaWorkspaceError(`${name} does not exist in workspace: ${path}`);
  return path;
}

export function normalizeWorkspace(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new OpaWorkspaceError('workspace must be an object');
  const rawFiles = value.files;
  if (!rawFiles || typeof rawFiles !== 'object' || Array.isArray(rawFiles)) throw new OpaWorkspaceError('workspace.files must be an object');
  ...
  if (entries.length > MAX_FILES) throw new OpaWorkspaceError(`workspace exceeds ${MAX_FILES} files`);

  const files = {};
  let totalBytes = 0;
  for (const [rawPath, source] of entries) {
    const path = validateWorkspacePath(rawPath);
    if (typeof source !== 'string') throw new OpaWorkspaceError(`Workspace file must be text: ${path}`);
    const bytes = Buffer.byteLength(source);
    if (bytes > MAX_FILE_BYTES) throw new OpaWorkspaceError(`Workspace file exceeds ${MAX_FILE_BYTES} bytes: ${path}`);
    totalBytes += bytes;
    if (totalBytes > MAX_WORKSPACE_BYTES) throw new OpaWorkspaceError(`workspace exceeds ${MAX_WORKSPACE_BYTES} bytes`);
    if (extname(path).toLowerCase() === '.json') {
      try { JSON.parse(source); }
      catch (error) { throw new OpaWorkspaceError(`Invalid JSON in ${path}: ${error.message}`); }
    }
    files[path] = source;
  }

  const entrypoints = value.entrypoints == null
    ? []
  ...
  return {
    ...
    activeFile: assertWorkspaceReference(value.activeFile, 'activeFile', files) || Object.keys(files)[0],
...
function safeChildEnv() {
  const names = ['PATH', 'HOME', 'TMPDIR', 'TEMP', 'TMP', 'LANG', 'LC_ALL', 'SystemRoot', 'WINDIR', 'PATHEXT'];
  const env = {};
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const append = (current, chunk, stream) => {
      ...
      if (next.length > MAX_OUTPUT_BYTES) {
        child.kill('SIGKILL');
        finishError(new OpaCliError(`OPA ${stream} exceeded ${MAX_OUTPUT_BYTES} bytes`, { code: 'OPA_OUTPUT_LIMIT' }));
        return current;
      }
      return next;
    };
    ...
    child.on('error', error => {
      if (error.code === 'ENOENT') {
        finishError(new OpaCliError(`OPA executable not found: ${opaBinary()}`, { code: 'OPA_UNAVAILABLE' }));
      } else {
        finishError(new OpaCliError(`Failed to start OPA: ${error.message}`, { code: 'OPA_START_FAILED' }));
      }
    });
    child.on('close', exitCode => {
      ...
      if (timedOut) {
        rejectPromise(new OpaCliError(`OPA command timed out after ${timeoutMs}ms`, { code: 'OPA_TIMEOUT', ...result }));
...
async function materialize(workspaceValue, callback) {
  ...
  try {
    ...
    return await callback({ root, workspaceDir, capabilitiesPath, workspace, pathFor: path => safeAbsolutePath(workspaceDir, path) });
  } finally {
...
function parseJsonOutput(text, label) {
  ...
  catch (error) { throw new OpaCliError(`OPA ${label} returned invalid JSON: ${error.message}`, { code: 'OPA_INVALID_OUTPUT', stdout: text }); }
}
...
function fallbackFinding(result) {
  const message = (result.stderr || result.stdout || `OPA exited with code ${result.exitCode}`).trim();
...
export async function checkWorkspace(workspace) {
  return materialize(workspace, async context => {
    ...
    let payload = null;
    try { payload = JSON.parse(result.stdout || result.stderr); } catch { /* use fallback */ }
    const findings = payload ? mapOpaErrors(payload, context.workspaceDir) : fallbackFinding(result);
...
export async function formatWorkspace(workspace) {
  return materialize(workspace, async context => {
    ...
    for (const path of Object.keys(files)) {
      ...
      if (extension === '.rego') {
        const result = await runOpa([
          'fmt', '--check-result', '--capabilities', context.capabilitiesPath, context.pathFor(path),
        ]);
...
function validateQuery(query) {
  if (typeof query !== 'string' || !query.trim()) throw new OpaWorkspaceError('query must be a non-empty string', 'OPA_QUERY_INVALID');
  if (query.length > 2048 || /[\0\r\n]/.test(query)) throw new OpaWorkspaceError('query is too long or contains control characters', 'OPA_QUERY_INVALID');
  return query.trim();
}
...
export async function evaluateWorkspace(workspace, query, { input, explain = 'notes' } = {}) {
  ...
  return materialize(workspace, async context => {
    let inputPath = context.workspace.inputFile ? context.pathFor(context.workspace.inputFile) : null;
    if (input !== undefined) {
    ...
    const args = [
      ...
      '--format=json',
      `--explain=${explainMode}`,
      '--capabilities', context.capabilitiesPath,
      ...dataArgs(context),
    ];
...
export async function testWorkspace(workspace) {
  return materialize(workspace, async context => {
    ...
    if (!paths.some(path => path.endsWith('.rego'))) throw new OpaWorkspaceError('workspace contains no Rego files');
    const common = ['--format=json', '--capabilities', context.capabilitiesPath, '--timeout', '5s', ...paths];
    const testsResult = await runOpa(['test', ...common], { allowFailure: true, timeoutMs: DEFAULT_TIMEOUT_MS });
    const coverageResult = await runOpa(['test', '--coverage', ...common], { allowFailure: true, timeoutMs: DEFAULT_TIMEOUT_MS });
    return {
...
export function dependencyProjection(payload, query) {
  const base = Array.isArray(payload?.base_documents) ? payload.base_documents : [];
  const virtual = Array.isArray(payload?.virtual_documents) ? payload.virtual_documents : [];
  const refs = [
  ...
  const unique = [...new Map(refs.map(entry => [`${entry.kind}:${entry.value}`, entry])).values()];
  return {
    nodes: [
      { id: 'query', label: query, kind: 'query' },
      ...unique.map((entry, index) => ({ id: `dependency-${index}`, label: entry.value, kind: entry.kind })),
    ],
    edges: unique.map((_, index) => ({ from: 'query', to: `dependency-${index}`, kind: 'depends-on' })),
  };
}
...
export async function readWorkspaceInput(workspace) {
  return materialize(workspace, async context => {
    ...
    if (extension !== '.json') throw new OpaWorkspaceError('Reading input in the UI currently requires a JSON inputFile');
