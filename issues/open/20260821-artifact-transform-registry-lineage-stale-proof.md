...
- [ ] a generic `ArtifactTransform` contract exists outside individual adapters.
- [ ] a generic transform registry can resolve and list the proof transform.
- [ ] duplicate/malformed transform registration is rejected deterministically.
- [ ] generic transform registry code contains no OPA/Dagu/Mermaid special-case branches.
- [ ] exactly one built-in deterministic transform is added for this proof.
- [ ] the proof transform consumes the existing generic GraphProjection path rather than re-parsing OPA or Dagu itself.
- [ ] the proof transform produces canonical Mermaid `ArtifactContent.kind = 'text'`.
- [ ] identical source content/revision produces byte-identical canonical Mermaid content.
- [ ] the derived result records `derivedFrom` source artifact id + revision.
- [ ] the derived result records transform id + transform version.
- [ ] source content can receive a deterministic fallback revision/hash without Git.
- [ ] unchanged source is reported current.
- [ ] changed source is reported stale.
- [ ] regeneration from changed source records the new revision and returns to current.
- [ ] OPA and Dagu GraphProjection behavior remains green.
- [ ] BPMN / Mermaid existing behavior remains green.
- [ ] local check/test/build gates are green.
