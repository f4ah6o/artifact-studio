...
**Next implementation slice:** add navigation from Architecture Graph nodes back to their source Artifact/entity. After that, continue provider coverage (for example n8n when available) or investigate a compiler-backed way to distinguish direct OPA reads from transitive dependency closure before introducing `reads`. Keep domain-specific impact-analysis semantics out of core.
