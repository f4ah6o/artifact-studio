...
- 2026-08-21: generic `discoverRelationships` runtime capability now merges adapter-derived ephemeral relationships with persisted authored relationships without storing derived edges. OPA is the first provider: `opa deps` virtual-document dependency closure becomes `depends-on` + `provenance=discovered` between resolvable OPA rule entities. Because `opa deps` is transitive closure, these edges are intentionally **not** labeled `reads`; `input.*` / base documents and unknown virtual documents are not fabricated as SemanticRefs. Architecture Graph entity resolution is cached once per artifact during each graph build.
- 2026-08-21: Dagu became the fourth `SemanticEntity` provider and second `discoverRelationships` provider. Stable named/id steps reuse the existing Dagu GraphProjection node identity; canonical `depends` declarations become direct `depends-on` + `provenance=discovered` edges from dependent step to dependency step. Anonymous steps remain structural-only, and unknown/ambiguous dependency targets are not fabricated. This provider is CLI-independent because the dependency data is explicit in canonical YAML.
- 2026-08-24: Architecture Graph nodes now navigate back to their source Artifact and, when the adapter supports it, focus the concrete SemanticEntity through an optional generic `revealSemanticEntity` runtime hook. BPMN selects/scrolls the model element, Dagu selects the visual step, Bonita BDM selects the Business Object and highlights the field, and OPA opens the defining Rego file and selects the package/rule head. Mermaid DOM details stay isolated inside the generic GraphProjection renderer.

**Next implementation slice:** continue provider coverage (for example n8n when available), add richer Architecture Graph filtering/traversal UI, or investigate a compiler-backed way to distinguish direct OPA reads from transitive dependency closure before introducing `reads`. Keep domain-specific impact-analysis semantics out of core.
