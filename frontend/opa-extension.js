...
function syncUi() {
  ...
  if (active) {
    ...
    setStatus(hasFiles() ? 'OPA workspace 準備完了' : 'OPA workspace — Rego / data / input を開いてください');
  } else {
...
function renderFindings(findings = []) {
  ...
  for (const finding of findings) {
    ...
    const location = [finding.file, finding.line, finding.column].filter(value => value != null).join(':');
...
function parseInputEditor() {
  ...
  catch (error) { throw new Error(`Input JSON: ${error.message}`); }
}
...
function addFile(path, source = '') {
  ...
  if (!safe || safe.includes('\\') || safe.startsWith('/') || safe.split('/').some(part => !part || part === '.' || part === '..')) {
    throw new Error('安全な相対パスを入力してください');
  }
  if (!/\.(rego|json|ya?ml)$/i.test(safe)) throw new Error('対応形式は .rego / .json / .yaml / .yml です');
...
async function importFiles(files) {
  ...
  if (list.length === 1 && list[0].name.toLowerCase().endsWith('.opa-workspace.json')) {
    ...
    const imported = parsed.content?.kind === 'workspace' ? parsed.content : parsed.workspace || parsed;
    if (!imported?.files || typeof imported.files !== 'object') throw new Error('OPA workspace JSON に files がありません');
    ...
    for (const file of list) {
      ...
      if (path.includes('\\') || path.startsWith('/') || path.split('/').some(part => !part || part === '.' || part === '..')) throw new Error(`安全でないパス: ${path}`);
...
function exportWorkspace() {
  ...
  download('policy.opa-workspace.json', `${JSON.stringify({ version: 1, adapter: 'opa', content: workspaceContent(workspace) }, null, 2)}\n`, 'application/json');
}
...
els.addFile.addEventListener('click', () => {
  try {
    ...
    els.newFile.value = '';
  } catch (error) { handleError('追加エラー', error); }
});
...
for (const [element, action, label] of [
  ...
  element.addEventListener('click', event => {
    ...
    action().catch(error => handleError(label, error)).finally(syncActionStates);
  }, true);
}
...
const codexObserver = new MutationObserver(() => { if (opaActive) syncActionStates(); });
