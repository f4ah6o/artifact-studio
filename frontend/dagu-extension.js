...
async function validate() {
  ...
  try {
    ...
    setStatus(result.ok ? 'dagu validate 完了' : `dagu validate: ${result.findings?.length || 0}件のエラー`);
  } catch (error) {
...
function syncUi() {
  ...
  if (!wasActive) {
    project().catch((error) => setStatus(`Dagu graph preview エラー: ${error.message}`));
    refreshCapabilities().catch((error) => setStatus(`Dagu capability 確認エラー: ${error.message}`));
  }
}
...
els.validate.addEventListener(
  ...
  (event) => {
    ...
    validate().catch((error) => setStatus(`Dagu validation エラー: ${error.message}`)).finally(syncActionStates);
  },
  true,
);
