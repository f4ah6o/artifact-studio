...
function buildEventDefinition(node, topLevelDefsMap) {
  ...
  if (marker === 'timer' && node.timerExpression) {
    ...
    const timerTag =
      te.type === 'date' ? 'timeDate' : te.type === 'cycle' ? 'timeCycle' : 'timeDuration';
...
function buildProcess(proc, defaultFlowMap, topLevelDefsMap) {
  ...
  for (const node of nodes) {
    ...
    if (node.calledElement && node.type === 'callActivity')
    ...
    if (node.loopType) {
      ...
      if (typeof node.loopType === 'object') {
        ...
        if (node.loopType.loopCondition) {
          loopAttrs.loopCondition = create('bpmn:FormalExpression', {
            body: node.loopType.loopCondition,
          });
        }
      }
      ...
      const mi =
        typeof node.multiInstance === 'object' ? node.multiInstance : { type: node.multiInstance };
      ...
      if (mi.completionCondition) {
        miAttrs.completionCondition = create('bpmn:FormalExpression', {
          body: mi.completionCondition,
        });
      }
    ...
    if (node.isExpanded && node.nodes && node.nodes.length > 0) {
      ...
      for (const child of node.nodes) {
        ...
        if (
          childEventDef &&
          childEl.$descriptor.properties.some((p) => p.name === 'eventDefinitions')
        ) {
  ...
  for (const edge of edges) {
    ...
    const needsCondition =
      ...
      (sourceNode &&
        ...
        sourceNode._direction === 'Diverging' &&
        !defaultFlowIds.has(eid));
    if (needsCondition) {
...
function buildDefaultFlowMap(proc) {
  ...
  for (const n of nodes) {
    if (
      (n.type === 'exclusiveGateway' || n.type === 'inclusiveGateway') &&
...
async function generateBpmnXml(lc, coordMap) {
  ...
  const { elements: topLevelElements, markerMap: topLevelDefsMap } = buildTopLevelDefinitions(
    processes,
    normalizedLc.definitions,
  );
