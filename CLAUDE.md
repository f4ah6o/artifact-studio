...
- **ElkJS Layered**: JavaScript port of the Eclipse Layout Kernel's layered algorithm. Our auto-layout engine — see `src/bpmn/layout.js`.
...
- **MCP**: Model Context Protocol — the protocol Claude Code uses to talk to external tools. We expose the generator via `src/server/mcp-bpmn-server.js`.
...
Production code is grouped by responsibility under `src/`; developer-only tooling is under `tools/`; executable tests are under `tests/`.
...
| File | Purpose |
|------|---------|
| `src/bpmn/pipeline.js` | Orchestrator + CLI + Public API (`runPipeline`) |
| `src/bpmn/rules.js` | Rule Engine: 28 rules, 4 layers (Soundness/Style/Pragmatics/Workflow-Net); M05/M06 severity=OFF. Verify count: `grep -c '^\s*id:' src/bpmn/rules.js` |
| `src/bpmn/validate.js` | Thin wrapper around `runRules()` |
| `src/bpmn/types.js` | `isEvent`, `isGateway`, `isArtifact`, `bpmnXmlTag` |
| `src/bpmn/utils.js` | `loadConfig`, `CFG`, constants, `esc`, `wrapText` |
| `src/bpmn/topology.js` | `inferGatewayDirections`, `sortNodesTopologically`, `orderLanesByFlow` |
| `src/bpmn/layout.js` | `logicCoreToElk`, `runElkLayout` (ElkJS Sugiyama) |
| `src/bpmn/coordinates.js` | `buildCoordinateMap`, `clipOrthogonal`, pool width balancing |
| `src/bpmn/bpmn-xml.js` | `generateBpmnXml` — OMG-compliant BPMN 2.0 XML + DI |
| `src/bpmn/svg.js` | `generateSvg` — SVG rendering of all BPMN elements |
| `src/bpmn/icons.js` | Event markers, task icons, bottom markers (Loop, MI, Ad-Hoc) |
| `src/bpmn/dot.js` | `logicCoreToDot` / `dotToLogicCore` — Graphviz DOT support |
| `src/bpmn/workflow-net.js` | WF-Net soundness checks (used by WF01–WF03 rules) |
| `src/bpmn/visual-refinement.js` | Optional compaction/refinement passes P1–P7.1 (off by default) |
| `src/bpmn/edge-simplify.js` | Post-process ELK edge waypoints to reduce zigzag bends |
| `src/bpmn/schema-gate.js` | `validateLogicCoreSchema` — ajv draft-2020-12 strict gate for the HTTP API |
| `src/bpmn/moddle-import.js` | BPMN XML → Logic-Core via bpmn-moddle (parallel to import.js) |
| `src/server/http-server.js` | HTTP API server (`/api/v1/generate`, `/orchestrate`, `/chat`) |
| `src/server/mcp-bpmn-server.js` | MCP server entry point |
| `src/ai/orchestrator.js` | Multi-agent orchestration (modeler → layout → reviewer → compliance) |
| `src/server/audit.js` | Append-only audit log (JSONL) |
| `src/server/delivery.js` | Webhook delivery + dead-letter queue |
| `tools/evaluate-slm.js` | Evaluation runner against fixture sets |
| `tools/prepare-training-data.js` | Training-data prep for SLM eval |
| `src/ai/` | 7 agent modules: chat, compliance, layout, llm-provider, modeler, prompt-sections, reviewer |
| `tools/robustness/` | Synthetic-data + benchmarking subsystem (9 modules + config; see `tools/robustness/README.md`) |
| `src/bpmn/import.js` | BPMN XML Parser → Logic-Core JSON |
| `src/bpmn/config.json` | Externalized constants (shapes, colors, spacing) |
...
vp test --run
vp build
node src/bpmn/pipeline.js tests/fixtures/simple-approval.json /tmp/test
```

After every change: the root Vite+ gates must pass.

### Adding a New Test

1. Place fixture in `tests/fixtures/` (JSON Logic-Core)
2. Add test in `tests/pipeline.test.js` (Vitest/Vite+, importing from `../src/bpmn/pipeline.js`)
3. For golden-file tests: place `.expected.bpmn` alongside the fixture

### Adding a New Rule

1. Insert rule object into `src/bpmn/rules.js` → `RULES` array
...
5. Add tests in `tests/pipeline.test.js`
...
1. Reproduce: `node src/bpmn/pipeline.js tests/fixtures/<fixture>.json /tmp/dbg`
...
3. Only after the diff is reviewed: regenerate via the fixture's documented procedure (typically `node src/bpmn/pipeline.js <fixture> <out>` and then `cp <out>.bpmn <fixture>.expected.bpmn`).
4. Commit golden updates in their own commit, separate from code changes.

### Extend the rule engine

1. Insert the rule object into the `RULES` array in `src/bpmn/rules.js`.
...
4. Add a positive and a negative fixture under `tests/fixtures/` and assertions in `tests/pipeline.test.js`.
5. Verify: `vp test --run tests/pipeline.test.js`.
...
2. **Re-validate downstream**: any change must still produce valid Logic-Core per `references/input-schema.json`. Run a sample text through the orchestrator (`src/ai/modeler.js`) and check the schema-validation step passes.
...
2. Sub-flags live in `src/bpmn/config.json` under `CFG.visualRefinement`: `dynamicLaneHeader`, `laneCompaction`, `edgeLabelCollisionRepair` (all on by default when `visualRefinement: true`). See `src/bpmn/visual-refinement.js` for the pass implementations.
3. Verify against goldens: `vp test --run tests/visual-refinement.test.js`.

### Run a robustness benchmark

1. Synthetic-data run: `node tools/robustness/cli.js run --target=lc-json`.
2. Multi-target run: `node tools/robustness/cli.js run --target=both` (LC-JSON + DOT paths through the LLM).
3. MaD subset validation: `node tools/robustness/cli.js mad-check`.
...
- BPMN config in `src/bpmn/config.json`, not hardcoded
...
- Schema-strict gate: every Logic-Core input at the HTTP entry passes through `src/bpmn/schema-gate.js` (ajv draft-2020-12) before reaching the pipeline. LLM output is never trusted raw.
- Body size cap: 10 MB. Rate limit: 30 req/min per IP. Both in `src/server/http-server.js`.
...
- **No hard-coded constants where `config.json` applies.** Shapes, colors, spacing, font metrics all live in `src/bpmn/config.json` and are loaded via `utils.js → CFG`. Hard-coding bypasses profile customization and tests.
...
node src/bpmn/pipeline.js input.json output-basename

# Stdin:
cat input.json | node src/bpmn/pipeline.js - output

# With DOT export:
node src/bpmn/pipeline.js input.json output --dot

# DOT → Logic-Core JSON:
node src/bpmn/pipeline.js graph.dot output --import-dot

# BPMN → Logic-Core (Round-Trip):
node src/bpmn/import.js existing.bpmn extracted.json

# With documentation export:
node src/bpmn/pipeline.js input.json output --doc

# Start MCP server:
node src/server/mcp-bpmn-server.js
```
