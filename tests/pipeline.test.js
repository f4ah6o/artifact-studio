...
} from '../src/bpmn/pipeline.js';
import { normalizeLaneAssignments } from '../src/bpmn/topology.js';
import { wrapText, wrapTextByPx } from '../src/bpmn/utils.js';
import { alignLinearFlowCrossAxis, messageFlowPorts } from '../src/bpmn/coordinates.js';
import {
  routeGatewayFlowsToCardinalPorts,
  routeCrossLaneFlowsByDirection,
} from '../src/bpmn/visual-refinement.js';

import { bpmnToLogicCore, bpmnToLogicCoreLegacy } from '../src/bpmn/import.js';
import { moddleParse, moddleToLogicCore } from '../src/bpmn/moddle-import.js';
import { checkWorkflowNetSoundness, bpmnToPN } from '../src/bpmn/workflow-net.js';
import { runRules, RULES, loadRuleProfile } from '../src/bpmn/rules.js';
import { logicCoreToDot, dotToLogicCore } from '../src/bpmn/dot.js';
import { parseBody, validateCallbackUrl } from '../src/server/http-server.js';
...
describe('Round-trip (JSON → BPMN → JSON)', () => {
  test('single process without id gets a stable process id and remains schema-valid', async () => {
    ...
    const { validateLogicCoreSchema } = await import('../src/bpmn/schema-gate.js');
...
describe('HTTP Server utilities', () => {
  ...
  test('validateCallbackUrlAsync rejects host that resolves to internal IP', async () => {
    ...
    const { validateCallbackUrlAsync, _setDnsLookup } =
      await import('../src/server/http-server.js');
  ...
  test('validateCallbackUrlAsync passes through sync errors unchanged', async () => {
    const { validateCallbackUrlAsync } = await import('../src/server/http-server.js');
...
describe('http-server production auth gate', () => {
  test('startupCheck refuses production without API key', async () => {
    const { startupCheck } = await import('../src/server/http-server.js');
...
describe('audit/dead-letter path configuration', () => {
  test('audit module defaults to os.tmpdir()/bpmn-generator/audit/', async () => {
    ...
    vi.resetModules();
    const { getAuditPath } = await import('../src/server/audit.js');
  ...
  test('audit module honors AUDIT_LOG_PATH env', async () => {
    ...
    vi.resetModules();
    const { auditLog, getAuditPath } = await import('../src/server/audit.js');
  ...
  test('delivery module honors DEAD_LETTER_PATH env', async () => {
    ...
    vi.resetModules();
    const { getDeadLetterDir } = await import('../src/server/delivery.js');
...
describe('Pass 3 metric assertions', () => {
  ...
  test('fewer label-vs-label bbox overlaps after refinement on dense fixture', async () => {
    ...
    const { estimateTextBBox, bboxOverlaps } = await import('../src/bpmn/visual-refinement.js');
...
import { logicCoreToElk, buildFlowPorts, resolveFlowPortLayout } from '../src/bpmn/layout.js';
