...
import { parseArgs, loadConfig, resolveEndpoint } from '../tools/robustness/cli.js';
...
const _fixturesDir = _resolve(__testDirname, 'fixtures');
...
import { enumerateCells, sampleCells } from '../tools/robustness/synthetic-generator.js';
...
import {
  ...
  buildLcJsonPrompt,
  extractJson,
} from '../tools/robustness/synthetic-generator.js';
...
import {
  ...
  canonicalSignature,
  isStructurallyEqual,
} from '../tools/robustness/graph-isomorphism.js';
...
import {
  ...
  runRoundtripCheck,
  runStressTest,
} from '../tools/robustness/stress-tester.js';
...
describe('robustness/stress-tester — runRoundtripCheck', () => {
  test('passes on simple LC that roundtrips cleanly', async () => {
    const lc = loadFixture('simple-approval.json');
    const { runPipeline } = await import('../src/bpmn/pipeline.js');
...
import { classify, computeFingerprint } from '../tools/robustness/failure-classifier.js';
...
import { persistFailure } from '../tools/robustness/fixture-persister.js';
...
import { generateReport, computeDrift } from '../tools/robustness/report-generator.js';
...
describe('robustness — end-to-end with mocked LLM and tmpfs', () => {
  ...
  test('valid LC sample passes through full pipeline', async () => {
    const { generateSamples } = await import('../tools/robustness/synthetic-generator.js');
    const { runStressTest } = await import('../tools/robustness/stress-tester.js');
    const { classify } = await import('../tools/robustness/failure-classifier.js');
    const { persistFailure } = await import('../tools/robustness/fixture-persister.js');
...
import { buildDotPrompt, extractDot } from '../tools/robustness/synthetic-generator.js';
...
import { runMadCheck } from '../tools/robustness/mad-validator.js';

describe('robustness/mad-validator — runMadCheck', () => {
  test('returns aggregate stats from a fixture subset directory', async () => {
    const subsetDir = _resolve(__testDirname, 'fixtures/mad-subset-test');
